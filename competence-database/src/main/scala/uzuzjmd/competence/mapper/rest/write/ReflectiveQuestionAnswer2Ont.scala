...
import uzuzjmd.competence.persistence.dao.{Competence, ReflectiveQuestion, ReflectiveQuestionAnswer, User}
...
object ReflectiveQuestionAnswer2Ont {
  def convert(reflectiveQuestionAnswer: ReflectiveQuestionAnswerData): Unit = {
    ...
    val question: ReflectiveQuestion = new ReflectiveQuestion(new Competence
    (reflectiveQuestionAnswer.getCompetenceId), reflectiveQuestionAnswer.getQuestionId)
